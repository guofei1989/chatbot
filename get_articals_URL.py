    ...
    if response.status_code == 200:  #检查状态响应码
        ...
        if data["code"] == 0:  #为0表示请求成功
            for article in articles:
                ...
                #打印文章信息
                print(f"pub time: {article['pub_time']}") 
                print(f"title: {article['title']}") 
                print(f"art url: {article['art_url']}")                  
                print(f"pic url: {article['pic_url']}") 
    
                art_data={"title": article['title'],"pub_time": article['pub_time'], 
                         "art_url": article['art_url'] , "pic_url": article['pic_url'],
                         'cursor': data["data"]["cursor"] }
                  
                datas = [art_data]       #放入列表中      
                df = pd.DataFrame(datas)
                df.to_csv('articles.csv',mode='a',index=False,header=0)  #保存为csv文件
            ...
            if "cursor" in data["data"]:  #如果存在游标，表示还有文章
                get_article_urls(wxid, key, data["data"]["cursor"])  #递归调用自身，新游标获取下一页数据 有没有办法标记游标
        else:
            print(f"Error: {data['msg']}")   #不为0 打印错误信息
    else:
        print("Failed to retrieve data from the server.")  #状态码不是200  打印错误信息
